...
test('Astro build generates session transcript pages with stable anchors and transcript states', async () => {
  ...
  const firstSessionHtml = await readFile('dist/sessions/001-ssg-choice/index.html', 'utf8');
  assert.match(firstSessionHtml, /id="entry-1"/);
  assert.match(firstSessionHtml, /June 5, 2026 at 8:39 PM UTC/);
  assert.match(firstSessionHtml, /class="session-entry session-entry--user"/);
  assert.match(firstSessionHtml, /class="session-entry session-entry--assistant"/);
  assert.match(firstSessionHtml, /Entry 1/);
  assert.match(firstSessionHtml, /Entry 2/);
  assert.doesNotMatch(firstSessionHtml, />#entry-1</);
  assert.doesNotMatch(firstSessionHtml, /Transcript highlights/);
  assert.doesNotMatch(firstSessionHtml, /<h2>Highlights<\/h2>/);
  assert.doesNotMatch(firstSessionHtml, /<h2 id="session-transcript-heading">Transcript<\/h2>/);
  assert.doesNotMatch(firstSessionHtml, /<dt>Source<\/dt>/);
  assert.doesNotMatch(firstSessionHtml, /<dt>Imported<\/dt>/);
  assert.doesNotMatch(firstSessionHtml, /<dt>Entries<\/dt>/);

  const researchLogsHtml = await readFile('dist/sessions/003-research-logs/index.html', 'utf8');
  assert.match(researchLogsHtml, /Show remaining 2 lines/);
  assert.match(researchLogsHtml, /Redaction review required/);
  assert.match(researchLogsHtml, /collapsed output/);
  assert.match(researchLogsHtml, /truncated/);
  assert.doesNotMatch(researchLogsHtml, /Transcript highlights/);
  assert.doesNotMatch(researchLogsHtml, /<h2>Highlights<\/h2>/);
});
